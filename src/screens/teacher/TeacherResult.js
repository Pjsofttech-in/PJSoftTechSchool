...
const StudentDetails = React.memo(({ details }) => (
  <View style={styles.detailsList}>
    {details?.map((d, i) => (
      <Text key={i} style={styles.detailText}>
        {d.subjectName}: <Text style={styles.boldText}>{d.obtainedMarks}</Text>/{d.maxMarks}
      </Text>
    ))}
  </View>
));
    ...
    <View style={styles.examCard}>
      <View style={styles.examHeader}>
        <View style={[styles.examTypeBadge, badgeStyle.badge]}>
          <Text style={[styles.examTypeText, badgeStyle.text]}>{item.examType}</Text>
        </View>
...
const ResultItemRow = React.memo(({ item, onOpenAssign, onOpenHistory }) => {
  ...
  return (
    <View style={styles.resultItem}>
      <View style={styles.resultMainRow}>
        <View style={styles.resultLeft}>
          ...
          <View style={styles.flexTextContainer}>
            ...
            {item.details && item.details.length > 0 && <StudentDetails details={item.details} />}
          </View>
        </View>
      ...
      <View style={styles.inlineActionRow}>
        ...
        >
          <MatIcon name="playlist-edit" size={16} color={PRIMARY} />
          <Text style={styles.inlineButtonText}>Assign Marks</Text>
        </Pressable>
        ...
        >
          ...
          <Text style={styles.inlineButtonText}>View History</Text>
        </Pressable>
      </View>
    </View>
  );
...
const TeacherResultContent = () => {
  ...
  const dynamicResultRenderer = useCallback(({ item }) => (
    <ResultItemRow item={item} onOpenAssign={openAssignMarks} onOpenHistory={openStudentHistory} />
  ), [openAssignMarks, openStudentHistory]);
              ...
              <FlatList
                ...
                ListEmptyComponent={
                  <View style={styles.emptyBox}>
                    ...
                    <Text style={styles.emptyText}>No registered students tracked inside matrix.</Text>
                  </View>
                }
              />
            )}
      ...
      >
        <View style={styles.dialogOverlay}>
          <View style={styles.dialogCard}>
            <Text style={styles.dialogTitle}>Assign Marks</Text>
            ...
            <View style={styles.dialogActions}>
              ...
              <Pressable style={styles.dialogBtn} android_ripple={SOLID_RIPPLE} onPress={submitAssignedMarks} disabled={assignLoading}>
...
const styles = StyleSheet.create({
  ...
  errorText: { color: OUTLINE, marginTop: 12, textAlign: 'center', fontSize: 14 },
  ...
  appbartitle: { flex: 1, fontSize: 20, color: '#1C1B1F', marginLeft: 8, fontWeight: '600' },
  
  sectionHeader: { paddingTop: 8, paddingBottom: 12 },
  sectionTitle: { fontSize: 14, color: PRIMARY, fontWeight: '700', letterSpacing: 0.15 },
  ...
  outlineButtonText: { color: PRIMARY, fontSize: 14, fontWeight: '600' },
  ...
  modalTitle: { fontSize: 18, fontWeight: '600', color: '#1C1B1F' },
  modalSub: { fontSize: 13, color: '#49454F', marginTop: 4 },
  ...
  examName: { fontSize: 15, fontWeight: '600', color: '#1C1B1F' },
  examDateRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 4 },
  examDate: { fontSize: 12, color: '#49454F' },
  ...
  subjectChipText: { fontSize: 12, color: PRIMARY, fontWeight: '600' },
  ...
  showResultBtnText: { color: '#fff', fontWeight: '600', fontSize: 14 },
  ...
  studentAvatarText: { color: PRIMARY, fontWeight: '700', fontSize: 16 },
  studentName: { fontSize: 14, fontWeight: '700', color: '#1C1B1F' },
  marksText: { fontSize: 13, color: '#49454F', marginTop: 4 },
  detailsList: { marginTop: 8, gap: 2 },
  detailText: { fontSize: 12, color: '#49454F' },
  ...
  inlineButtonText: { fontSize: 13, color: PRIMARY, fontWeight: '600' },
  inlineDivider: { width: 1, backgroundColor: SURFACE_VARIANT },
  
  emptyBox: { alignItems: 'center', paddingVertical: 32 },
  emptyText: { color: OUTLINE, marginTop: 8, fontSize: 14, textAlign: 'center' },
  ...
  dialogTitle: { fontSize: 20, fontWeight: '600', color: '#1C1B1F' },
  dialogSub: { fontSize: 14, color: '#49454F', marginBottom: 16, marginTop: 4 },
  inputGroup: { marginBottom: 12 },
  inputLabel: { fontSize: 12, fontWeight: '600', color: PRIMARY, marginBottom: 4 },
  textInputAndroid: { borderWidth: 1, borderColor: OUTLINE, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8, fontSize: 14, color: '#1C1B1F' },
  ...
  dialogBtnText: { fontSize: 14, fontWeight: '700', color: PRIMARY },
  ...
  timelineTitle: { fontSize: 14, fontWeight: '700', color: '#1C1B1F' },
  timelineMeta: { fontSize: 12, color: '#49454F', marginTop: 2 }
});